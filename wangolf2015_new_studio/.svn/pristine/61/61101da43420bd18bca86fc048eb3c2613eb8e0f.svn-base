...
public class UserFriendsInfoActivity extends BaseActivity implements OnClickListener {
    ...
    public void getData() {
        dialog.show();
        try {
            ServiceFactory.getCommunityEngineInstatice().getUserInfo(user_id,friend_id , page, number, new IOAuthCallBack() {
                @Override
                public void getIOAuthCallBack(String result) {

                    if (result.equals(ConstantValues.FAILURE)) {
                        ToastUtils.showInfo(UserFriendsInfoActivity.this, ConstantValues.NONETWORK);
                    } else {
                        UserInfoNewEntity bean = GsonTools.changeGsonToBean(result, UserInfoNewEntity.class);
                        if (bean.getStatus() == 1) {
                            if (bean.getData() != null) {
                                UserInfoNewEntity data = bean.getData().get(0);
                                setUserInfo(data);// 更新页面
                            }
                        } else {
                            ToastUtils.showInfo(UserFriendsInfoActivity.this, bean.getInfo());
                        }
    ...
    public void switchFrom() {
        switch (flag) {
            case 0:
                ...
                break;
            case 1:
                bean = (UserMyFriendsEntity) getIntent().getSerializableExtra("bean");
                if (bean != null) {
                ...
                break;
            case 2:
                beanf = (UserMyFansEntity) getIntent().getSerializableExtra("bean");
                if (beanf != null) {
    ...
    public void setUserInfo(UserInfoNewEntity bean) {
        ...
        if (bean != null) {
            ...
            mTitle.setText(CheckUtils.checkEmpty(bean.getNick_name()) ? bean.getMobile() : bean.getNick_name());
