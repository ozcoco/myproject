...
public class OrderListActivity extends BaseActivity implements OnClickListener {
	...
	private OrderListAdapter<BaseOrderEntity> adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (adapter == null) {
			adapter = new OrderListAdapter<BaseOrderEntity>(this);
	...
	public void getData() {
		...
		try {
			ServiceFactory.getIUserEngineInstatice().getOrderList(user_id, type, page, number, new IOAuthCallBack() {
				...
				private List<BaseOrderEntity> list;
	...
	public void onClick(View v) {
		...
		case R.id.rb_practice:
			...
			page = 1;
			ismore = false;
			List<BaseOrderEntity> practice_lists = (List<BaseOrderEntity>) adapter.getmListItems();
		...
		case R.id.rb_ball:
			...
			page = 1;
			ismore = false;
			List<BaseOrderEntity> ball_lists = (List<BaseOrderEntity>) adapter.getmListItems();
			if (ball_lists != null)
		...
		case R.id.rb_shop:
			...
			page = 1;
			ismore = false;
			List<BaseOrderEntity> shop_lists = (List<BaseOrderEntity>) adapter.getmListItems();
			if (shop_lists != null)
