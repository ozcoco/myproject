...
public class GetUserPwdActivity extends BaseActivity implements OnClickListener {
	...
	public void onClick(View v) {
		...
		case R.id.upPasswowd:
			...
			password = DigestUtils.md5Hex(ed_npassword.getText().toString().trim());
			if (phone.length() != 11) {
				Toast.makeText(this, "请输入正确的手机号码", 0).show();
				return;
			}
	...
	public void getResetPwdCode() {
		try {
			ServiceFactory.getIUserEngineInstatice().getResetPwdCode(phone, new IOAuthCallBack() {
	...
	public void doResetPwd() {
		dialog.show();
		try {
			ServiceFactory.getIUserEngineInstatice().doResetPwd(phone, password, code, new IOAuthCallBack() {
	...
	protected void updateButton() {
		bt_get_checknum.setBackgroundColor(getResources().getColor(R.color.gray));
			...
			public void handleMessage(Message msg) {
				...
				if (msg.what == 1) {
					...
					if (time == 0) {
						...
						bt_get_checknum.setBackground(getResources().getDrawable(R.drawable.bt_green_yuan_all_selector));
					} else {
