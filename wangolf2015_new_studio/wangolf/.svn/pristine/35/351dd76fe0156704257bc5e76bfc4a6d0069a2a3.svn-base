...
public class UserEventListActivity extends BaseActivity implements OnClickListener {
	...
	private EventListAdapter<BaseOrderEntity> adapter;
	private ArrayList<? extends BaseOrderEntity> data;
	...
	private List<BaseOrderEntity> list;// 参加的活动
	private ArrayList<PublishEventEntity> my_event; // 发起的活动
	private int flag = 0;// 0为参加的活用1为发起的活动 用于判断跳转
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (adapter == null) {
			adapter = new EventListAdapter<BaseOrderEntity>(this);
		...
		pull_refresh_list.getRefreshableView().setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				case 0:
					// 参加的活动跳转
					OrBallListEntity bean = (OrBallListEntity) adapter.getItem(position);
					if (bean != null) {
				...
				public void getIOAuthCallBack(String result) {
					if (result.equals(ConstantValues.FAILURE)) {
						...
						if (bean.getData().size() == 0) {
							...
							data = (ArrayList<? extends BaseOrderEntity>) bean.getData();
							list = (List<BaseOrderEntity>) adapter.getmListItems();
							if (isR) {
								list.clear();
								list.addAll(data);
							} else {
								if (list != null & ismoredata) {
									...
									adapter.setmListItems((List<BaseOrderEntity>) data);
								}
							}
	...
	public void getMyPublishEvent() {
		dialog.show();
		try {
			ServiceFactory.getIUserEngineInstatice().getMyPublishEvent(user_id, page, number, new IOAuthCallBack() {
				...
				public void getIOAuthCallBack(String result) {
					if (result.equals(ConstantValues.FAILURE)) {
						...
						PublishEventEntity bean = GsonTools.changeGsonToBean(result, PublishEventEntity.class);
						if ("1".equals(bean.getStatus())) {
							if (bean.getData().size() == 0) {
								...
								ArrayList<PublishEventEntity> data = bean.getData();
							...
							ToastUtils.showInfo(UserEventListActivity.this, bean.getInfo());
						}
	...
	public void onClick(View v) {
		...
		case R.id.rb_event:
			...
			rb_event.setBackground(getResources().getDrawable(R.drawable.rd_message_l));
			type = 3;
			page = 1;
			flag = 0;
			isR = false;
		...
		case R.id.rb_PublishEvent:
			setRadioButton();
			rb_PublishEvent.setTextColor(getResources().getColor(R.color.white));
			rb_PublishEvent.setBackground(getResources().getDrawable(R.drawable.rd_message_r));
			page = 1;
			flag = 1;
			isR = false;
			ismoredata = false;
	...
	public void setRadioButton() {
		rb_event.setBackgroundColor(getResources().getColor(R.color.transparent));
		rb_PublishEvent.setBackgroundColor(getResources().getColor(R.color.transparent));
		rb_event.setTextColor(getResources().getColor(R.color.common_text_h1));
		rb_PublishEvent.setTextColor(getResources().getColor(R.color.common_text_h1));
