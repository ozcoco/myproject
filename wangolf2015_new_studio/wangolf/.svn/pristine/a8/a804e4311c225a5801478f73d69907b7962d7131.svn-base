  ...
  private RelativeLayout mRelayout;
			...
			ServiceFactory.getBallEngineInstatice().findBallSearch(cityid, date, ballname, longitude, latitude, order, type, new IOAuthCallBack() {
				...
				public void getIOAuthCallBack(String result) {
					if (result.equals(ConstantValues.FAILURE)) {
						...
						ArrayList<BallDetailEntity> datas = GsonTools.changeGsonToBean(result, BallDetailEntity.class).getData();
						if (datas.size()>0) {
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		if (adapter.getItem(arg2) != null) {
			...
			BallDetailEntity bean = adapter.getItem(arg2);
			intent.putExtra("BallDetailEntity", bean);
			intent.putExtra("ballname", bean.getBallname());
