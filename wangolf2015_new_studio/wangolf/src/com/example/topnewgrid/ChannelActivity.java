 ...
 */
public class ChannelActivity extends Activity implements OnItemClickListener, OnClickListener {
    /**
     * 用户栏目的GRIDVIEW
     */
    ...
    private void initData() {
        ...
        channelManage = new ChannelManage(LocationApplication.getApp().getSQLHelper(),tableName);
            ...
            case R.id.userGridView:
                ...
                if (position !=-1) {
                    ...
                    if (moveImageView != null) {
                        ...
                        //判断是否发生了双击
                        if(position != 0 && position != 1&mLastTime != 0 && mCurTime - mLastTime < 500) {
    ...
    private View getMoveView(ViewGroup viewGroup, View view, int[] initLocation) {
       // Log.i("wangolf","获取移动的VIEW，放入对应ViewGroup布局容器");
    ...
    private void saveChannel() {
        channelManage.getManage(LocationApplication.getApp().getSQLHelper(), tableName).deleteAllChannel(tableName, true);
        ...
        for (int i = 0; i < userAdapter.getChannnelLst().size(); i++) {
            ...
            ConstantValues.title_name[i] = data.get(i).getName();
            ConstantValues.title_id[i] = data.get(i).getId();
        }
        ConstantValues.title_size = userAdapter.getChannnelLst().size();
...
private void  setTagReuslt(int position){
    Intent result = new Intent();
    result.putExtra("position",position);
    setResult(ConstantValues.NEWSTAGS, result);
    saveChannel();
    ConstantValues.newsTags = true;
    if (flagsize != ConstantValues.title_size || start_lastid != end_lastid) { // 大小不相等 频道有增加或减少
        ConstantValues.newsTags = true;
