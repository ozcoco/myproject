...
import me.wangolf.ballprac.bean.PromptInfo;
...
public class BallToBuyActivity extends BaseActivity implements OnClickListener
{
	...
	public void initData()
	{
		...
		bean = (BallInfoEntity.DataEntity.DistributorsEntity) getIntent()
				.getSerializableExtra("balldestrientity");
		tv_brief.setText(ballname);
		tv_date.setText(date.replace("%20", " "));
		price = Double.valueOf(bean.getPrice());
		all_price = price;
	...
	public void toBuy()
	{
		...
		if (CheckUtils.checkEmpty(tv_contact.getText().toString().trim()) | tv_contact
		...
		order_bean.setSupplier_id(bean.getSupplier_id());// 供应商ID
		...
		order_bean.setProduct_id(Integer.valueOf(bean.getId()));// ???产品ID
					...
					.toPrepay(order_bean, new IOAuthCallBack()
					{
						...
						public void getIOAuthCallBack(String result)
						{
							...
							try
							{
								...
								PromptInfo bean = GsonTools.changeGsonToBean(result, PromptInfo.class);
												
								if ("1".equals(bean.getStatus()))
								{														
									sn = bean.getData().get(0).getOut_trade_no();
									...
									if (sn != null)
									{
										...
										{
											ToastUtils.showInfo(getBaseContext(), order_bean.getType());
										}
