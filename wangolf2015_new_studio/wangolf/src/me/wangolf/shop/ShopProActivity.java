...
public class ShopProActivity extends Activity implements View.OnClickListener
{
	...
	public void getData()
	{
		try
		{
			...
			ServiceFactory.getShopEngineInstatice()
					.getProInfo(proid, new IOAuthCallBack()
					{
						@Override
						public void getIOAuthCallBack(String result)
						{
							// ToastUtils.showInfo(getApplicationContext(),
							// result);
							...
							{
								bean = GsonTools
										.changeGsonToBean(result, InfoPro.class);

								if (bean.getData().size() != 0)
								{

									InfoPro.DataEntity data = bean.getData()
											.get(0);

									tv_infopro_name.setText(data.getSub_name());

									tv_pro_pronprice.setText("￥" + data
											.getCurrent_price());

									tv_pro_prodis
											.setText(data.getDiscount() + "折");

									tv_proinfo_prooprice.setText("价格：￥" + data
											.getOrigin_price());

									tv_infopro_procount.setText("已出售" + data
											.getBuy_count() + "件");

									tv_infopro_prodetail.setText(data
											.getBrief());

									sharetitle = data.getSub_name() + "  " + data
											.getDiscount() + "折" + " " + "￥" + data
											.getOrigin_price();
									...
									String[] url = data.getImg_list()
											.split(",");
									
									Log.i("banner图链接", data.getImg_list());
									
//									if (!CheckUtils.checkEmpty(url[0]))
//									{
//										picfile = url[0].substring(0, url[0]
//												.lastIndexOf(".")) + "_180_180" + url[0]
//												.substring(url[0]
//														.lastIndexOf("."));
//									}
									...
									for (int i = 0; i < url.length; i++)
									{
										...
										if (!CheckUtils.checkEmpty(path))
										{
											...
											path = s[0].substring(0, s[0]
													.lastIndexOf(".")) + "_640" + s[0]
													.substring(s[0]
															.lastIndexOf("."));			
										}
									...
									//
									// JSONArray jsonProArr =
									// info.getJSONArray("proattr");
									//
									// if (jsonProArr != null)
									// {
									// JSONObject jsonColor = (JSONObject)
									// jsonProArr.get(0);
									//
									// color = jsonColor.getString("0");
									//
									// JSONObject jsonSize = (JSONObject)
									// jsonProArr.get(1);
									//
	...
	private void initDot(int size)
	{
		...
		for (int i = 0; i < size; i++)
		{
			LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(CommonUtil
					.dip2px(getApplicationContext(), 6), CommonUtil
					.dip2px(getApplicationContext(), 6));
